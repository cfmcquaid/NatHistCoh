...
state <- c(E=100000, Q=0, K=0, S=0, Z=0, C=0, Y=0, M=0)
...
regr <- function(t, state, parameters){
  with(as.list(c(state, parameters)), {
   ...
   list(c(dE, dQ, dK, dS, dZ, dC, dY, dM))
  })
}
# Calculations
...
outA$rate <- outA$S*paramA["Sc"] / rowSums(outA[2:9]); outW$rate <- outW$S*paramW["Sc"] / rowSums(outW[2:9]); outS$rate <- outS$S*paramS["Sc"] / rowSums(outS[2:9]); outF$rate <- outF$S*paramF["Sc"] / rowSums(outF[2:9]);
...
Orate <- out[which(out$source == Osource & out$variable %in% c("rate") & out$time %in% c(1:Otime)), ]
